...
LINEAR_SPEED = 0.025 # speed for everything
KP = 1.5/1000 # proportional controller constant
TURN_THRESH = 60 # amount line needs to turn to initiate turn
TURN_SPD = 0.11 # default turn speed
TURN_SPD_AGGR = 0.16 # more aggressive turn speed
TURN_SPD_CHILL = 0.07 # less aggressive turn speed
MORE_AGGR_THRESH = 160 # line for robot turning faster (line is at top of screen)
LESS_AGGR_THRESH = 320 # line for robot turning slower (line is at bot of screen)
...
class ImageSubscriber:
    ...
    def listener_callback(self, msg):
        ...
        # lower = (0, 0, 200)
        # upper = (180, 30, 255)
        lower = (200, 200, 200) # thresholding lower vals (RGB)
        upper = (255, 255, 255)
        ...
        if self.state["IDLE"]:
            ...
            if line:
                ...
                if abs(line['angle']) < TURN_THRESH:
                    ...
                    cmd.angular.z = (float(error) * - KP) # prop controller for turn radius
            ...
            if line:
                ...
                if abs(line['angle']) < TURN_THRESH:
                        if line['y'] < LESS_AGGR_THRESH: # line is at top of screen, need to turn less aggressively
                            cmd.angular.z = TURN_SPD_CHILL
                            cmd.linear.x = LINEAR_SPEED
                        elif line['y'] > MORE_AGGR_THRESH: # line is at bottom of screen, need to turn more aggressively
                            cmd.angular.z = TURN_SPD_AGGR
                            cmd.linear.x = LINEAR_SPEED
                        else:
            ...
            if line:
                ...
                if abs(line['angle']) < TURN_THRESH:
                    if line['y'] < LESS_AGGR_THRESH: 
                        cmd.angular.z = -1 * TURN_SPD_CHILL
                        cmd.linear.x = LINEAR_SPEED
                        rospy.loginfo("running")

                    elif line['y'] > MORE_AGGR_THRESH:
                        cmd.angular.z = -1 * TURN_SPD_AGGR
                        cmd.linear.x = LINEAR_SPEED
                    else:
                        cmd.angular.z = -1 * TURN_SPD
                        cmd.linear.x = LINEAR_SPEED
                else:
                    self.switch_case("FOLLOW")
            else:
